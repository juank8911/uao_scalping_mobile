...
async function fetchWithAuth<T = any>(url: string, options: RequestInit = {}): Promise<T> {
  ...
  if (response.status === 401) { await deleteToken(); notifyAuthRequired(); throw new Error('Unauthorized'); }
  const text = await response.text();
  const data = text ? (() => { try { return JSON.parse(text); } catch { return text; } })() : null;
  if (!response.ok) throw new Error(data?.detail || data?.message || `HTTP ${response.status}`);
  return data as T;
}
export const login = async (username: string, password: string) => { const body = new URLSearchParams({ username, password }); const response = await fetch(`${API_BASE_URL}/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: body.toString() }); const data = await response.json(); if (!response.ok) throw new Error(data?.detail || 'Login failed'); if (data.access_token) await saveToken(data.access_token); return data; };
export const refreshToken = () => fetchWithAuth(`${API_BASE_URL}/auth/refresh`, { method: 'POST' });
export const logout = async () => { try { await fetchWithAuth(`${API_BASE_URL}/auth/logout`, { method: 'POST' }); } finally { await deleteToken(); } };
...
export const getGlobalHistory = async (limit = 20): Promise<{ data: GlobalTradeRecord[] }> => { const data = await fetchWithAuth<any>(`${API_BASE_URL}/history?${query({ limit })}`); return data?.data ? data : { data: data || [] }; };
