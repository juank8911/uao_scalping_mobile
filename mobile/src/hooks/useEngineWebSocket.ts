...
const wsUrl = () => { const base = API_BASE_URL.replace(/^http/, 'ws').replace(/\/$/, ''); return `${base}/ws/engine`; };
...
const scheduleReconnect = () => { if (reconnect || consumers <= 0) return; const delay = Math.min(1000 * 2 ** reconnectAttempt++, MAX_RECONNECT_MS); reconnect = setTimeout(() => { reconnect = null; void connect(); }, delay); };
...
const connect = async () => { if (consumers <= 0 || socket?.readyState === WebSocket.OPEN || socket?.readyState === WebSocket.CONNECTING) return; const token = await getToken(); if (!token) { useEngineStore.getState().setConnected(false); return; } const url = token ? `${wsUrl()}?token=${encodeURIComponent(token)}` : wsUrl(); socket = new WebSocket(url); socket.onopen = () => { reconnectAttempt = 0; useEngineStore.getState().setConnected(true); if (subscribedSymbol) send({ action: 'subscribe', symbol: subscribedSymbol, timeframe: subscribedTimeframe }); stopHeartbeat(); heartbeat = setInterval(() => send({ action: 'ping', timestamp: Date.now() }), HEARTBEAT_MS); }; socket.onmessage = (event) => { try { handleMessage(JSON.parse(event.data)); } catch (error) { console.error('[WS] Invalid message', error); } }; socket.onerror = () => { /* onclose gestiona la reconexión; no cerrar manualmente durante el handshake */ }; socket.onclose = () => { stopHeartbeat(); socket = null; useEngineStore.getState().setConnected(false); scheduleRevoid connect(); }; };
